...
export default function UploadPage() {
  ...
  async function loadFriends() {
    ...
    try {
      ...
      // Your backend returns users with id, display_name etc.
      // We'll treat those as already usable.
  ...
  async function uploadSingleFile(file: File, sessionId: string) {
    ...
    const uploadMeta = await apiFetch("/media/upload-url", {
      ...
      body: JSON.stringify({
        ...
        session_id: sessionId, // ✅ CHANGED: pass session id so feed can batch this upload
      }),
    });
  ...
  async function startUpload() {
    ...
    // ✅ NEW: one session id per upload run (this is what makes batching work)
    ...
    try {
      ...
      // Refresh folder list (in case upload created new session groups, etc.)
      await loadFolders();
      ...
      <div style={styles.container}>
        ...
        <div style={styles.header}>
          ...
          <div style={styles.subtitle}>
            Upload photos and videos into your CertainShare library.
          </div>
        </div>
        ...
        <div style={styles.card}>
          ...
          <div style={styles.section}>
            <div style={styles.sectionTitle}>Choose media</div>
          ...
          {isSingle && (
            <div style={styles.section}>
              <div style={styles.sectionTitle}>Memory note (optional)</div>
          ...
          {/* FOLDER */}
          <div style={styles.section}>
            <div style={styles.sectionTitle}>Where should this go?</div>

            {loadingFolders ? (
              <div style={styles.helperText}>Loading albums...</div>
            ) : (
              <select
                ...
                style={styles.select}
              >
                <option value="">Individual Upload</option>
                {folders.map((f) => (
            ...
            <div style={styles.helperText}>
              If you choose an album, the album privacy settings will override
              your selection.
            </div>

            <Link href="/new-album" style={styles.linkButton}>
              + Create new album
            </Link>
          </div>
          ...
          {/* VISIBILITY */}
          <div style={styles.section}>
            <div style={styles.sectionTitle}>Who can see this?</div>

            {selectedFolder ? (
              <div style={styles.lockBox}>
                <div style={{ fontWeight: 900 }}>
                  Album privacy is locked:{" "}
          ...
          <div style={styles.section}>
            ...
            <div style={styles.helperText}>
              Mobile-only uploads are only viewable on mobile. This helps screenshot
              detection.
            </div>
          </div>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  subtitle: { marginTop: 8, fontSize: 14, color: "var(--muted)", lineHeight: 1.5 },
  card: {
    borderRadius: 22,
    background: "white",
  ...
  linkButton: { display: "inline-block", marginTop: 10, textDecoration: "none", fontWeight: 950, fontSize: 13, color: "#2563eb" },
  ...
  primaryButtonDisabled: { cursor: "not-allowed", opacity: 0.6, boxShadow: "none" },
};